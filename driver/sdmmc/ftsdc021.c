        ...
        u8 RespType, u8 InhibitDATChk, u32 Argu)
{
    ...
    u32 err;
    u8 wait;
//    sdc_dbg_print("***cmd%d cmdType=%d DataPresent=%d\n", CmdIndex, CmdType, DataPresent);
    ...
    if (InhibitDATChk || (RespType == SDHCI_CMD_RTYPE_R1BR5B)) {
        if (gpRegSDC(ip_idx)->PresentState & SDHCI_STS_CMD_DAT_INHIBIT) {
            sdc_dbg_print(" ERR## CMD DATA INHIBIT is one.\n");
            return 1;
        }
    ...
    if (err != 0)
        sdc_dbg_print("<ipx=%d> command error %x\n", ip_idx, err);
    return err;
}
...
//            && (gpRegSDC(ip_idx)->PresentState & SDHCI_STS_CARD_INSERT)) {
...
ftsdc021_set_bus_speed_mode(u8 ip_idx, u8 speed)
{
    ...
    if (speed < 4) {
        if (speed < 3) {
            ...
            //SDHost[ip_idx].Card->max_dtr = 50000000;// << speed;
        } else { /* current controller stable @ max 130 MHz */
            SDHost[ip_idx].Card->max_dtr = SDHost[ip_idx].max_clk;
        }
    } else
...
//        goto loop;
         ...
         */
        // TODO: remove the following code to support SD Card only
        ...
        if (SDHost[ip_idx].Card->CardType == SDIO_TYPE_CARD) {
            ...
            if (err) {
                ...
                if (SDHost[ip_idx].Card->Memory_Present == 1) {
                    ...
                    sdc_dbg_print(" ERR## Combo card is not support in SDhost driver ... \n");
                    return 1;
                } else {
                    SDHost[ip_idx].Card->CardType = SDIO_TYPE_CARD;
                }
...
#define SDIO_CLK_SEL   0 //1     /* 1:syspll, 0:xtal */
#if SDIO_CLK_SEL
#define SDIO_CLK_SRC   (96 * 1000 * 1000)
#else
#define SDIO_CLK_SRC   (24 * 1000 * 1000)
#endif
...
//        ftsdc021_SetPower(ip_idx, 21);
    ...
    //sdc_dbg_print("clk = %d ,timeount ms = %d\n",gm_pmu_get_axi_clk(),SDHost[ip_idx].Card->FlowSet.timeout_ms);
